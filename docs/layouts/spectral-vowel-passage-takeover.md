...
- DECIDED after the first full check (136 copies, 122 passed, every control in place): Output level
  widens to +48 so the quiet captures (breathing copy 1, held copy 1) can match, and the ~2 dB
  crossfade dip in 12 Sweep/Shuffle copies is accepted (check tolerance 3 dB). Rozaya: *"Widen it, and
  honestly, it *shouldn't* matter. if it does end up mattering I can just readjust later"*.
