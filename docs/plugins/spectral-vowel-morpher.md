...
> **All three are global here, where Passage puts harmonic and depth per slot.** Passage models a singer moving through a route of separate notes; Morpher is one continuous field, so the tract holds one shape across the whole morph. The *harmonic* can additionally be overridden per layer — see **Layer overtone harmonic**.
...
(seconds)` became `Drift period (sec / min / beats by Rate Mode)` in the same change
...
than bolted onto the end — **Rate Mode** beside the Auto-morph time it governs,
