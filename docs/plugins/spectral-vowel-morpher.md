...
> **This shapes the voice, not the wash.** The wash's overtone is one shared spectrum for the whole instrument, and giving each layer its own would mean an extra spectral pass per layer per grain — real CPU on a plugin that already runs out at around four layers. At Texture 0 the per-layer harmonic is the whole effect; as you move toward the wash end the layers converge on the global setting.
