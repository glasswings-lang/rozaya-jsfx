...
**Ramp by (slider 91)** `-20000 to +20000, step 0.001, default 0` (units match the selected target)
...
**Ramp duration (slider 93)** `0–60 minutes, default 0` — **per-target.** How long the *selected* target takes to travel from its baseline to baseline + `by`. Each target has its own duration; a target with duration 0 does not ramp (so set a duration for every target you want to move). · **Ramp start delay (slider 97)** `0–60 minutes, default 0` — **per-target.** Wait this many minutes (after engage) before *this* target begins moving. Stagger targets by giving them different start delays. · **Ramp engage (all targets, slider 96)** `Off / On, default Off` — **global.** One switch arms the whole wind-down; every configured target then rides its own duration after its own start delay.
...
**Drift up amount (slider 83)** `0 to 20000, default 0` (units match target)
...
**Drift down amount (slider 84)** `0 to 20000, default 0` (units match target)
...
**Drift period (slider 85, cycles)** `0–1000, default 8, 0 = off`
...
**Drift shape (slider 87)** `Sine / Triangle / Random, default Sine`
