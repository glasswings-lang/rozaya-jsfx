...
For **BPM** the delta is in whatever unit the BPM slider is currently showing, which depends on Rate Mode. In **Own BPM** it's BPM (`-60` ramps 120 → 60). In **Host x** it's a multiplier delta matching the slider: at BPM `1`, a `by` of `-0.5` ends at `0.5` — half speed, still locked to the project. Numbers are small in that mode; the 0.1 step is for nudging by ear. Landing the delta on the multiplier rather than on the finished BPM is what lets a Host x ramp **stretch with the project tempo** instead of being a fixed amount that means something different at every tempo.
