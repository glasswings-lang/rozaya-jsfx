...
**Ramp by (slider 76)** `-20000 to +20000, step 0.1, default 0` (units match the selected target)
...
**Ramp duration (slider 78)** `0–60 minutes, default 0` — **per-target.** How long the *selected* target takes to travel from baseline to baseline + `by`. Each target has its own; a target with duration 0 does not ramp (set a duration for every target you want to move). · **Ramp start delay (slider 82)** `0–60 minutes, default 0` — **per-target.** Wait this many minutes after engage before *this* target begins moving (stagger targets by giving them different delays). · **Ramp engage (all targets, slider 81)** `Off / On, default Off` — **global.** One switch arms the whole wind-down; each configured target then rides its own duration after its own start delay.
...
**Drift up amount (slider 68)** `0 to 20000, default 0` (units match target)
...
**Drift down amount (slider 69)** `0 to 20000, default 0` (units match target)
...
**Drift period (slider 70, beats)** `0–1000, default 8, 0 = off`
...
**Drift shape (slider 72)** `Sine / Triangle / Random, default Sine`
...
between 2 and 61 shifted up by one, so the twelve per-note groups then started at
slider 13 rather than 12. (They start at slider 15 now, after later additions.)
...
was illegible, the BPM slider had to be **hidden** in Host x and a menu shown
instead; because it was hidden, entering Host x had to **stamp** a landing value
into it. That stamp is the bug that cost a session elsewhere in the suite, where a
saved project had its hand-set rate overwritten on every load.

Beats per cycle is legible on its own, so the chain unwinds: nothing hides, nothing
is stamped, and BPM is always visible showing the value it is running at. (Suite
rule R13-revised, 2026-09-02; converted here 2026-09-04.)

**Drift and Ramp amounts stay in real BPM in both modes** — they are added to the
resulting speed, not to the beats-per-cycle number, so "wander by 10 BPM" means the
same thing whichever mode you are in.
