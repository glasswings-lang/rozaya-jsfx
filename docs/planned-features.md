...
# Bilateral / note-locked pan modes for the sequencer plugins — LANDED 2026-09-02

**Shipped and ear-tested the same day** in `polyrhythm_phase`, `polyrhythm_phase_v3`
and `melody_phase`. It went in as **twelve enum options and nothing else** — no new
sliders in any plugin, so no migration and no saved project moved.

Two things were got wrong on the way and are worth keeping:

- **Do not invent a name for a concept the suite already has.** The first build
  called it "Bilateral". The suite already had `Alternating`, in four plugins
  (`Full_Feature_Tremolo`, both sweeping filters, `rhythm-track`), with a whole
  `percycle_pan()` family behind it — Distributed / Converging / Diverging and
  their Ping-pong variants. The right move was to port that function verbatim and
  change only the TICK: the filters advance on their LFO wrap, Polyrhythm on the
  voice's tremolo wrap, Melody on the note trigger.
- **The pan index must be PER VOICE in Polyrhythm.** The filters have one pan
  position because a filter on a stereo bus can only have one. Copying that gave
  a single index stepped by the first active voice — i.e. panning at the base
  rate and ignoring the other seven. Rozaya heard it immediately: *"what I heard
  was it using the base rate, and ignoring the rest"*, and *"for something like
  polyrhythm preserving polyrhythm is the entire point"*. Each voice now steps on
  its own wrap.

**And the rule that killed two invented sliders** (Rozaya: *"Add to the slider that
reaches sideways... you could extend to infinity"*): the enum is the free axis.
`Pan cycle steps` was unnecessary because the positions are the ACTIVE VOICES, the
same geometry Spread already uses; a per-voice/linked switch was unnecessary
because voices sharing a tremolo rate already move as one pulse. **Before adding a
slider, check whether the plugin already knows the answer.**
