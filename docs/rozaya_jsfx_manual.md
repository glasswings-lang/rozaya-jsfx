...
- **Half-sine** — full-wave rectified sine remapped to bipolar (`2 · |sin(phase)| − 1`). Even-harmonic-only character, hollow and vaguely reedy. Distinct from any of the other waveforms here. **Sounds an octave higher than the other waveforms at the same note + octave setting.** This isn't a tuning bug — full-wave rectification produces a spectrum with no fundamental and its lowest partial at 2× the carrier frequency, so the perceived pitch is one octave up by design. To get the "expected" pitch from Half-sine, drop the Center Octave by 1 (or per-voice Semitones by 12). To play it stacked an octave above the other waveforms intentionally, leave the controls alone — that's the natural shape. Also carries a small DC offset (mean ≈ 0.27) which speakers don't reproduce; tremolo and pan attenuate it further.
