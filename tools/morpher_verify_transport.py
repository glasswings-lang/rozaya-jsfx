"""Verify the Morpher TRANSPORT slider migration -- by NAME, against a snapshot.
...
SNAP = r'E:\reaper\finished\backups\snapshots\_pre-morpher-transport-20260904'
MIG  = os.path.join(os.environ.get('TEMP', '.'), 'morpher-transport-migrated')
OLD_REV = 'c5ac335:src/spectral_vowel_morpher.jsfx'
NEW_SRC = 'src/spectral_vowel_morpher.jsfx'

DECL = re.compile(r'^slider(\d+):([^<]*)<([^,]*),([^,]*),([^>]*)>(.*)')

# AUTHORED rename table: new label -> old label. Matching by name is what makes
# this check independent of the migration's own permutation, so a control that
# was RENAMED in the same change has to be paired up explicitly. Inferring the
# pairing (nearest match, common prefix) would be the script exercising
# judgement, which is the thing tools here are not allowed to do.
RENAMED = {
    'Drift period (sec / beats when synced)': 'Drift period (seconds)',
}
...
def main():
    ...
    # apply the authored renames before pairing
    for new_label, old_label in RENAMED.items():
        assert new_label in new_by_name, 'rename table names a label the NEW layout does not have: ' + new_label
        assert old_label in old_by_name, 'rename table names a label the OLD layout does not have: ' + old_label
        old_by_name[new_label] = old_by_name.pop(old_label)
